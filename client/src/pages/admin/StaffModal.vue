<template>
  <a-modal :open="open" :title="props.title" @cancel="closeModal">
    ...
    <a-form :model="staff" ref="staffForm" name="staffForm" autocomplete="off">
      <a-form-item label="Mã nhân viên" name="codeStaff" :label-col="{ span: 24 }" :rules="rules.code">
        <a-input v-if="staff" v-model:value="staff.codeStaff" />
      </a-form-item>
      <a-form-item label="Tên nhân viên" name="nameStaff" :label-col="{ span: 24 }" :rules="rules.name">
      ...
      <a-form-item label="Email FPT" name="emailFpt" :label-col="{ span: 24 }" :rules="rules.emailFpt">
...
const rules = {
  code: [
    ...
    { pattern: /^[a-zA-Z0-9]+$/, message: "Mã nhân viên không được chứa ký tự đặc biệt!" }
  ],
