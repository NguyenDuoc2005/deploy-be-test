<template>
    <BreadcrumbDefault label="Quản lý công việc">
        <TodoListFilter :searchQuery="statetodo.search.searchQuery" @update:searchQuery="updateSearchQueryTodo" />
    ...
    <TodoListImportModal :open="state.isOpenModalImport" :title="'Import Task'" @close="handleCloseModalImport"
        @fetch-all="fetchTodoProject" @update-log="handleNewLog" />
    ...
    >
      <HistoryImportTodoModalTable
        ...
        :paginationParams="{ page: statetodo.paginationParamsHistory.page, size: statetodo.paginationParamsHistory.size }"
...
import { downloadExcelTodoList, downloadImportLogTodoList,getAllHistoryTodo, getAllTodoList, getAllTodoProject, historyImprortTodo, ParamsGetTodoList, TodoListProjectResponse, TodoProjectResponse } from '@/services/api/manage/todolist/todolist.api';
...
const fetchTodoProject = async () => {
    try {
        ...
        statetodo.todo = statetodo.todo.filter((value, index, self) =>
            index === self.findIndex((t) => (
                t.idTodo === value.idTodo
            ))
        );
...
const fetchHistory = async () => {
  try {
    const response = await getAllHistoryTodo()
...
watch(() => [statetodo.search.searchQuery, statetodo.search.level, statetodo.search.status], () => {
    statetodo.paginationParams.page = 1;
    debouncedFetchTodoProject();
});

const updateSearchQueryTodo = ({newQuery, newLevel, newStatus}: 
    {
        newQuery: string, 
