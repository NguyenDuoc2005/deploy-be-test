<template>
    <DivCustom label="Danh sách công việc " customClasses="mt-5">
    <template #icon>
        ...
        <div class="min-h-[360px]">
            ...
            }" :scroll="{ x: 1000, y: 300 }" @change="handlePageChange">
                <template #bodyCell="{ column, record }">
                    ...
                    <template v-if="column.key === 'nameTodo'">
                        ...
                        <a-input v-else v-model:value="editedTitle" size="large" @pressEnter="saveTitle(record)"
                            @blur="saveTitle(record)" class="w-full"
                            style="min-width: 120px; border: none; outline: none;" placeholder="Nhập tiêu đề thẻ" />
                    </template>
                    ...
                    <template v-if="column.key === 'namePhaseProject'">
                        <a-select v-model:value="record.namePhaseProject" style="min-width: 120px"
                        @change="(idPhase) => handlePhaseChange(record, idPhase)"
                        >
                            <!-- Dropdown options lấy từ API fetchPhases -->
                            <a-select-option v-for="phase in phases" :key="phase.idPhase"
                                :value="phase.idPhase">
                    ...
                    <template v-if="column.key === 'operation'">
                        <div class="flex flex gap-1 justify-center">
                            <a-tooltip title="Chi tiết">
                                <a-icon @click="handleNavigate(record.phaseId, record.idTodo)">
            ...
            <MaTodoMemberModal v-model="showAddMemberModal" @close="showAddMemberModal = false"
...
import { getAllPhaseMa, TodoProjectResponseMember, updateTodoPhaseProject } from '@/services/api/member/phase/phase.api'
...
const columns = [
    ...
    { title: 'Sprint', key: 'namePhaseProject', dataIndex: 'namePhaseProject', width: 150, align: 'center' },
    ...
    {
        title: 'Ngày tạo', key: 'createdDate', dataIndex: 'createdDate', width: 150, align: 'center',
        customRender: ({ text }) => text ? h(Tag, {}, () => dayjs(Number(text)).format('DD/MM/YYYY HH:mm')) : ''
    },
    {
        title: 'Ngày Cập nhật', key: 'lastModifiedDate', dataIndex: 'lastModifiedDate', width: 150, align: 'center',
        customRender: ({ text }) => text ? h(Tag, {}, () => dayjs(Number(text)).format('DD/MM/YYYY HH:mm')) : ''
    },
    {
        title: 'Hành động',
