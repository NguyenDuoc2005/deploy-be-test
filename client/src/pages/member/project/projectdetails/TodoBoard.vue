    ...
    <main class="flex-1 h-full overflow-x-auto overflow-y-hidden">
      <div>
        <div v-if="!isView" class="flex gap-6 items-start min-h-[100%] w-max p-3" ref="boardContainer">
          <draggable v-model="filteredBoard" group="columns" item-key="id" animation="300" ghost-class="column-ghost"
                ...
                <div class="w-64 bg-white/95 rounded-xl shadow-lg pt-2 p-1 flex flex-col flex-shrink-0"
                  ...
                  <div class="flex items-center justify-between mb-1">
                    <div class="flex-1">
                      ...
                      <template v-else>
                        <h2
                          class="text-base ml-2 font-semibold tracking-normal text-gray-700 hover:text-gray-900 cursor-pointer transition-all duration-200 ease-in-out"
                          style="font-family: 'Inter', sans-serif" @click="startEditTitle(element)">
                          {{ element.name }}
                        </h2>
                      </template>
                    </div>
                  ...
                  <div
                    ...
                    <draggable v-model="element.tasks" group="tasks" item-key="id" animation="200" ghost-class="ghost"
                      ...
                      <template #item="{ element: task }">
                        <div
                          ...
                          <div class="p-3 space-y-2">
                            ...
                            <!-- Name -->
                            <div class="ml-1 mr-1 text-sm font-medium text-gray-700 truncate leading-tight">
                              {{ task.name }}
                            </div>
                            ...
                            <div class="flex flex-wrap items-center gap-2 text-xs text-gray-600">
                              ...
                              <a-tooltip title="Tiến độ công việc " v-if="task.progress > 0">
                                <div v-if="task.progress !== null && task.progress !== undefined"
                                  class="px-2 py-0.5 font-semibold rounded-full flex items-center" :class="{
                                    'bg-blue-100 text-blue-700': task.progress !== 100,  // Khi chưa hoàn thành
                                    'bg-green-100 text-green-700': task.progress === 100  // Khi hoàn thành
                                  }">
                                  <!-- Kiểm tra tiến độ, nếu hoàn thành thì hiển thị dấu tích -->
        ...
        <TodoDetailModal v-model="showTaskDetailModal" :todoId="selectedTodoId" @close="closeDetailModal" />
      </div>
    </main>
  </BackgroundBoard>
</template>
...
watchEffect(() => {
  ...
  const result = meBoard.value.map(board => {
    const filteredTasks = board.tasks.filter(task => {
      // Nếu filters không có dữ liệu hoặc không có thay đổi nào, giữ lại tất cả task
      if (!filters || Object.keys(filters).length === 0 || Object.values(filters).every(value => !value)) {
        return true
      }
      ...
      // Lọc theo thẻ không có thành viên 
      if (filters.noMember && hasMember) return false;


      // Lọc theo thẻ của tôi 
      if (filters.myTodo) {
        if (!hasMember) return false; //  Không có member → loại luôn
        const isMyTask = task.memberTodo.some(
          member => member && member.id === userLogin.userId
        );
      ...
      // Lọc theo nhãn (nếu có)
      if (filters.labels && filters.labels.length > 0) {
        const hasLabelFilter = filters.labels.some(label => {
          switch (label) {
            case 'no_label':
        ...
        if (!hasLabelFilter) return false
      }

      // Lọc theo nhãn được chọn từ combobox (labelSelected là mảng idLabel)
      if (filters.labelSelected && filters.labelSelected.length > 0) {
        const taskLabelIds = task.labels.map(l => l.id)
        const matched = filters.labelSelected.some(labelId => taskLabelIds.includes(labelId))
        if (!matched) return false
      }
      ...
      if (filters.dueDate.length > 0) {
        const matchDue = filters.dueDate.some(dateFilter => {
          switch (dateFilter) {
            case 'no_due':
              return task.deadline === null // Không có hạn
            case 'overdue':
              return task.deadline > 0 && task.deadline < now  // Quá hạn và chưa xong
            case 'not_done':
              return !task.completionTime
            case 'done_early':
              return task.completionTime !== null && task.completionTime < task.deadline;// hoàn thành sớm
            case 'done_late':
              return task.completionTime !== null && task.completionTime > task.deadline;// hoàn thành muộn
            default:
              return false
          }
        })
        // Nếu không match bất kỳ điều kiện nào trong dueDate -> loại task đó
        if (!matchDue) return false
      }

      return true // Nếu không rơi vào trường hợp nào, giữ lại task
    })
