...
const uploadImage = async () => {
    ...
    isUploading.value = true; // Bật trạng thái loading

    try {
        const urlImage = await uploadFile(selectedFile.value);
        const payload = {
            urlImage: urlImage.data,
            nameFileOld: imageName.value,
        ...
        isUploading.value = false; // Tắt loading sau khi tải xong hoặc có lỗi
    }
};
...
const closeModal = () => {
    ...
    previewUrl.value = "";
    errors.value = { name: "", image: "" };
    isUploading.value = false; // Đảm bảo reset loading khi đóng modal
};
</script>