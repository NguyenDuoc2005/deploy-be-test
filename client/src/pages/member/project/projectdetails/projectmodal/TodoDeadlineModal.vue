<template>
    <a-modal :zIndex="10000" v-model:open="visible" title="Ngày hạn" @cancel="handleCancel" :footer="null" :width="350">
        <div class="mb-4">
            ...
            <div v-if="deadlineError" class="text-red-500 text-sm mt-1">Vui lòng chọn ngày hạn!</div>
        </div>
...
const deadlineError = ref(false);  // Đặt biến lỗi cho ngày hạn
...
const handleSave = () => {
    if (!deadline.value) {
        ...
        // toast.error('Vui lòng chọn hạn công việc trước khi lưu!');
        return;
    }
...
const handleDelete = () => {
    ...
    emit('updateSuccessDeadline');
    toast.success('Hủy hạn công việc...!');
    visible.value = false;
};
