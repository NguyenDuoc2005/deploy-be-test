<template>
    <div class="min-h-[360px]" style="margin: 10px; height: 560px; background-color: white;">
        <a-table :columns="columns" :data-source="todo" :rowKey="record => record.idTodo" :pagination="{
            current: paginationParams.page,
            pageSize: paginationParams.size,
            total: totalItems,
            showSizeChanger: true,
            pageSizeOptions: ['10', '20', '30', '40', '50']
        }" :scroll="{ x: 500, y: 400 }" @change="handlePageChange">
            <template #bodyCell="{ column, record }">

                <template v-if="column.key === 'todoName'">
                    <div v-if="editingTodoId !== record.idTodo" @click="startEditingTitle(record)"
                        class="cursor-pointer hover:bg-gray-100 p-1 rounded">
                        {{ record.todoName || 'Không có tiêu đề' }}
                    </div>

                    <a-input v-else v-model:value="editedTitle" size="large" @pressEnter="saveTitle(record)"
                        @blur="saveTitle(record)" class="w-full" style="min-width: 120px; border: none; outline: none;"
                        placeholder="Nhập tiêu đề thẻ" />
                </template>

                <template v-if="column.key === 'assignee'">
                    <div class="flex items-center flex-wrap gap-2" @click="openMemberModal(record)">
                        <a-tooltip v-for="member in [...record.students, ...record.staff]" :key="member.id"
                            :title="member.email">
                            <a-avatar :src="member.image" :alt="member.name" />
                        </a-tooltip>
                        <!-- <a-button shape="circle" icon="+"  /> -->
                        <div v-if="(!record.students || record.students.length === 0) && (!record.staff || record.staff.length === 0)"
                ...
                <template v-if="column.key === 'label'">
                    <div class="flex items-center flex-wrap gap-2">
                        <a-tag v-for="label in record?.label" :key="label.id"
                            :style="{ backgroundColor: `${label.color}`, color: getTextColor(label.color) }"
                            class="cursor-pointer" @click="openLabelModal('edit', record.idTodo)">
                            {{ label.name }}
                        </a-tag>
                ...
                <template v-if="column.key === 'todoListName'">
                    <a-select v-model:value="record.todoListName" style="min-width: 120px"
                        @change="(todoListId) => handleSelectChange(record, todoListId)">
                        <a-select-option v-for="todolist in todoList" :key="todolist.todoListId"
                ...
                <template v-if="column.key === 'priorityLevel'">
                        <a-select v-model:value="record.priorityLevel" style="min-width: 120px;"
                            @change="(value) => selectPriority(record, value)">
                            <a-select-option v-for="(label, key) in statusPriorityLevelOptions" :key="key" :value="key">
        ...
        <MaTodoMemberModal v-model="showAddMemberModal" @close="showAddMemberModal = false" :todoId="selectedTodoId" />

        <TodoTableLabelModal :visible="isLabelModalVisible" :labelData="selectedLabel" :mode="modalMode"
            @close="handleCloseLabelModal" @save="handleSaveLabel" @delete="handleDeleteLabel" :id="selectedTodoId" />
    </div>
</template>
...
import { MBMeUpdateIndexTodoRequest, MBMeUpdateTodoRequest, webSocketTodo } from '@/services/service/member/metodo.socket'
import MaTodoMemberModal from '@/pages/manage/todolist/MaTodoMemberModal.vue'
import { getAllPhaseMa, getAllTodoListByPhaseProject, getIndexMaxTodo, TodoListBoardResponse, TodoPhaseProjectResponse, TodoProjectResponse, updateTodoPhaseProject } from '@/services/api/manage/todolist/todolist.api'
...
watch(() => meAllDetailTodo?.value?.completionTime, (newVal) => {
...
const columns = [
    ...
    {
        title: 'Trạng thái', key: 'statusTodo', dataIndex: 'statusTodo', width: 150, align: 'center',
        customRender: ({ text }) => {
            const statusMap = {
            ...
            return h(Tag, { color: statusMap[text]?.color || 'default' }, () => statusMap[text]?.label || text);
        }
    },
    { title: 'Danh sách', key: 'todoListName', dataIndex: 'todoListName', width: 150, align: 'center' },
    ...
    { title: 'Sự ưu tiên', key: 'priorityLevel', dataIndex: 'priorityLevel', width: 150, align: 'center' },

    {
        title: 'Ngày hết hạn', key: 'deadlineTodo', dataIndex: 'deadlineTodo', width: 150, align: 'center',
        customRender: ({ text }) => text ? h(Tag, {}, () => dayjs(Number(text)).format('DD/MM/YYYY HH:mm')) : ''
    },
