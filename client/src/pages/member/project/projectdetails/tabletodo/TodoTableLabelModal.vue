<template>
  <a-modal :zIndex="10000" v-model:visible="modalVisible" title="Danh sách nhãn" :footer="null" @cancel="closeModal" centered width="400px">
    <!-- Ô TÌM KIẾM -->
    ...
    <div v-for="label in filteredLabels" :key="label.id" class="flex items-center justify-between mb-2">
      <div class="flex items-center flex-1">
        <a-checkbox v-model:checked="selectedLabelsMap[label.id]" class="mr-2" @change="toggleLabel(label.id, $event.target.checked)" />
        <div class="rounded-md px-2 py-1 text-sm font-semibold text-white w-full cursor-pointer" :style="{ backgroundColor: label.colorLabel }" @click="toggleLabel(label.id, !selectedLabelsMap[label.id])">
          {{ label.name }}
        </div>
      </div>
      <div class="flex space-x-1 ml-2">
        ...
        <a-popconfirm :zIndex="10000" title="Bạn có chắc muốn xóa nhãn này không?" ok-text="Xóa" cancel-text="Hủy" @confirm="deleteLabel(label.id)">
    ...
    <!-- Modal Thêm/Sửa nhãn -->
    <a-modal :zIndex="10001" v-model:visible="editModalVisible" :title="isEditing ? 'Chỉnh sửa nhãn' : 'Thêm nhãn'" :footer="null"  @cancel="closeModalChild" centered >
      <!--  Xem trước nhãn -->
      <div class="mb-3">
        <label class="block text-sm font-semibold mb-1">Xem trước:</label>
        <div class="rounded-md px-2 py-1 text-sm font-semibold text-white w-full" :style="{ backgroundColor: editLabelData.colorLabel || '#ccc' }">
      ...
      <div class="mb-3">
        ...
        <div class="grid grid-cols-5 gap-2">
          <div v-for="color in colorOptions" :key="color" @click="selectColor(color)" class="w-10 h-10 rounded cursor-pointer border-2 transition-all duration-150" :class="{ 'border-black scale-105': editLabelData.colorLabel === color }" :style="{ backgroundColor: color }"></div>
        </div>
